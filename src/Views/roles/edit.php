...
<div class="max-w-xl mx-auto mt-8 bg-white rounded-xl shadow p-8">
    ...
    <?php endif; ?>
    <form method="post" action="/roles/update" class="space-y-4">
        <input type="hidden" name="id" value="<?= $role['id'] ?>">
        <div>
            <label class="block text-sm font-semibold mb-1">Nombre</label>
            <input type="text" name="name" value="<?= htmlspecialchars($role['name']) ?>" class="form-input w-full rounded-lg border border-gray-300 bg-gray-50 focus:bg-white focus:border-primary focus:ring-primary px-4 py-2" required>
        </div>
        <div>
